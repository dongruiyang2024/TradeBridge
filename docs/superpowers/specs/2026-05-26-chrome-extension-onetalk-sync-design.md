...
- Web 工作台能创建人工回复，Chrome 插件在用户已登录的 OneTalk 页面上下文中代发，并回写 sent/failed 状态。
- 全链路不保存、不上传、不日志打印 OneTalk 登录鉴权。

## 4. 非目标

- 不通过 TradeBridge 服务端代持 OneTalk Cookie。
- 不由 TradeBridge 服务端代持凭据或直接调用 OneTalk 发送接口。
- 不做批量群发、自动营销或绕过人工确认的自动发送。
...
Chrome Extension
  ...
  - 调用 OneTalk 会话与消息接口
  - 领取 TradeBridge 待发消息并在 OneTalk 页面上下文中发送
  - 本地转换、去敏、游标管理
        |
        v
...
```json
{
  ...
  "web_accessible_resources": [
    {
      "resources": ["content/onetalk-page-script.js"],
      "matches": ["https://onetalk.alibaba.com/*"]
    }
  ],
  "action": {
...
- 封装 `/collector/v1/outbound-messages` 和 `/collector/v1/outbound-messages/:id/delivery`。
...
export interface TradeBridgeClient {
  ...
  markOutboundMessageDelivered(result: OutboundDeliveryResult): Promise<OutboundMessage>;
}
```

### 8.6 Outbound 发送链路

发送消息采用“Web 排队、扩展代发、服务端回执”的边界：

```text
TradeBridge Web
  POST /internal/v1/conversations/:id/outbound-messages
        |
        v
outbound_message(status=queued)
        |
        v
Chrome Extension background
  GET /collector/v1/outbound-messages
        |
        v
OneTalk content/page bridge
  IcbuIM.IMBaaSSDK.default.getMessageService().sendUIMessages(...)
        |
        v
Chrome Extension background
  POST /collector/v1/outbound-messages/:id/delivery
```

约束：

- 仅发送销售在 TradeBridge Web 当前会话中手动输入的文本。
- 服务端不接触 OneTalk Cookie、token 或页面 SDK 对象。
- Collector device 只能领取其绑定 seller account 的 queued 消息。
- 发送失败时保留 `failed` 状态和错误码，便于 Web 工作台回看。
