...
const customerQuery = "sellerAccountExternalId=seller-1";
...
test("assignment and follow-up update routes reject invalid scopes", async () => {
  ...
  assert.deepEqual(updateResponse.json(), { ok: false, error: "follow_up_task_not_found" });
});
